...
############## Enemy React for distance reaction, enemy hit for attack animation/type (fireball or arrow...)
    ...
    def is_colliding(self, poly):
        ...
        return (player_right > poly.left and player_left < poly.right) and (player_bottom > poly.top and player_top < poly.bottom)
 ...
 # Gravity effect (pull down)
    ...
    def interactableCollision(self, interact, player, lifesystem, scoresystem):
        ...
        if ((playerRightOffset > interact.xcorner and playerLeftOffset < interact.right) and
            ...
            elif interact.name.lower() == "key":
                if(playerRightOffset > interact.xcorner and playerLeftOffset < interact.right and 
                   playerBottomOffset > interact.ycorner and playerTopOffset < interact.bottom):
                    print("Player has the key")
                    player.hasKey = True
                    self.message = "You have collected the key"
                    self.messageTimer = 1000
                    return "key"
            
            elif interact.name.lower() == "door":
                if(playerRightOffset > interact.xcorner and playerLeftOffset < interact.right and 
                   playerBottomOffset > interact.ycorner and playerTopOffset < interact.bottom):
                    if player.hasKey:
                        print("Door opens")
                        self.message = "The door is unlocked. Welcome to Level 2"
                        self.messageTimer = 1000
                        return "door"
                    else:
                        print("The door is locked. Find the key")
                        self.message = "The door is locked. You need to find the key"
                        self.messageTimer = 1000
                        return None
...
class Game:
    def __init__(self, firstStart=True):
        if firstStart:
            ...
            self.start_button = self.frame.add_button("Start", self.button_handler, 200)
            self.restart_button = self.frame.add_button("Restart", self.restart_handler, 200)
        ...
        self.lvl2image = simplegui.load_image("https://github.com/RyanQuayum/pythongame/blob/main/level%202.jpg")
        self.menuImage = simplegui.load_image("https://github.com/RyanQuayum/pythongame/blob/main/opening%20page.PNG")
        self.deadImage = simplegui.load_image("https://github.com/RyanQuayum/pythongame/blob/main/end%20page.PNG")
        ...
        polygon_positions = [
            ...
            #x corner, y corner, width, height
        ...
        self.level = [
            { #level 1 configuration
                "background_image" :  "https://raw.githubusercontent.com/RyanQuayum/pythongame/refs/heads/main/lvl1.png",
                "player_start_pos" : Vector(100, 20),
                "enemies":[
                     enemy(Vector(70, 172), self.skeletonWalkRight, self.skeletonWalkLeft, self.skeletonAttackRight, self.skeletonAttackLeft, self.skeletonIdleRight, self.skeletonLeftIdle, self.skeletonDeadRight, self.skeletonDeadLeft, self.skeletonHitRight, self.skeletonHitLeft, 18),
                     rangedEnemy(Vector(900, 130), self.skeletonWalkRight, self.skeletonWalkLeft, self.skeletonAttackRight, self.skeletonAttackLeft, self.skeletonIdleRight, self.skeletonLeftIdle, self.skeletonDeadRight, self.skeletonDeadLeft, self.skeletonHitRight, self.skeletonHitLeft, 18)
                ],
            ...
            { # level 2 configuration
                "background_image" :  "https://github.com/RyanQuayum/pythongame/blob/main/level%202.jpg",
                "player_start_pos" : Vector(100, 20),
                "enemies": [],
                "interactables": [],
                "polygon positions":[
    ...
    def restart(self):
        self.current_level = 0
        self.gameState = "mainMenu"
        self.player.pos = Vector(100, 20)

        self.message = ""
        self.messageTimer = 0

        level_config = self.level[0]

        self.lvl1image = simplegui.load_image(level_config["background_image"])
        self.enemies = level_config.get("enemies", [])
        interactable_positions = level_config.get("interactables", [])
        self.interactables = [interactable(x, y, w, h, name) for x, y, w, h, name in interactable_positions]
        self.polygons = [inviswall(x, y, w, h) for x, y, w, h in level_config.get("polygon_positions", [])]
    ...
    def transition_to_next_level(self):
        ...
        if self.current_level >= len(self.level):
            print("There are no more level")
            self.gameState = "gameCompleted"
            return
        
        level_config = self.level[self.current_level]
        self.lvl1image = simplegui.load_image(level_config["background_image"])
        self.player.pos = level_config["player_start_pos"]
        ...
        self.messageTimer = 500


    def button_handler(self): # Start Button Handler
        if self.gameState == "mainMenu":
            self.gameState = "game"
            self.start_button.set_text("Menu")
    ...
    def restart_handler(self): #Restart Button Handler
        self.restart()
        self.start_button.set_text("Start")
    ...
    def draw(self, canvas):
        if self.gameState == "mainMenu":
            if self.menuImage.get_width() > 0 and self.menuImage.get_height() > 0:
                canvas.draw_image(self.menuImage, (640, 360), (1280, 720), (640, 360), (1280, 720))
            else:
                canvas.draw_text("Loading Main Menu...", (500, 360), 36, 'White')
            ...
            if self.deadImage.get_width() > 0 and self.deadImage.get_height() > 0:
                canvas.draw_image(self.deadImage, (640, 360), (1280, 720), (640, 360), (1280, 720))
            else:
                canvas.draw_text("Loading Dead Screen...", (500, 360), 36, 'White')
            ...
            #Drawing polygons and the interactables
            for polygon in self.polygons:
                polygon.draw(canvas)
            for interactable in self.interactables:
                interactable.draw(canvas)
            ...
            for interactable in self.interactables:
                interaction_result = self.interaction.interactableCollision(interactable, self.player, self.lifesystem, self.scoresystem)
                
                if self.messageTimer <= 0:
                    if interaction_result == "key" and not self.player.hasKey:
                        if self.interaction.is_colliding(interactable):
                            self.player.hasKey = True

                            self.message = "You have collected the key"
                            self.messageTimer = 1000

                    elif interaction_result == "door":
                        if self.interaction.is_colliding(interactable):
                            if self.player.hasKey:
                                if self.message != "Welcome to Level 2":
                            
                                    self.message = "Welcome to Level 2"
                                    self.messageTimer = 500
                                    self.transition_to_next_level()
                        else:
                            if self.message != " Find the key to open the door":
                            
                                self.message = "Find the key to open the door"
                                self.messageTimer = 500      

            print(f"Interaction Result: {interaction_result}, Player has key: {self.player.hasKey}") 
            ...
            if self.messageTimer > 0:
                canvas.draw_text(self.message, (640, 360), 48, "Red")
                self.messageTimer -= 1
            else:
                self.message = ""          
